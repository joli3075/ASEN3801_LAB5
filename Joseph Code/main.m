...
[t, state_vector1] = ode45(@(t, y) AircraftEOM(time, aircraft_state_array, aircraft_surfaces, wind_inertial, aircraft_parameters), time, Aircraft_State0);

% call PlotAircraftSim
fig = [211,212,213,214,215,216];

state_vector1 = state_vector1';
PlotAircraftSim(t, state_vector1, repmat(aircraft_surfaces, 1, numel(t)), fig, '-g');
...
% Call the ODE solver for the aircraft equations of motion
[t, state_vector2] = ode45(@(t, y) AircraftEOM(time, aircraft_state_array, aircraft_surfaces, wind_inertial, aircraft_parameters), time, Aircraft_State0);
...
% Call the ODE solver for the aircraft equations of motion
[t, state_vector3] = ode45(@(t, y) AircraftEOM(time, aircraft_state_array, aircraft_surfaces, wind_inertial, aircraft_parameters), time, Aircraft_State0);
...
%% 2 FINAL - combined trim and disturbance
time = [0, 10000000]; % Example time span from 0 to 100 seconds
Aircraft_State0 = [ 0, 0 , -18000 , 0 , 0.2780 , 0 , 20.99 , 0 , 0.5837 , 0 , 0 , 0 ] + [ 0, 0 , -18000 , deg2rad(15) , deg2rad(-12) , deg2rad(270) , 19 , 3 , -2 , deg2rad(0.08) , deg2rad(-0.2) ,0] ;
aircraft_state_array = Aircraft_State0; 
...
% Define Aircraft Surface Controls
aircraft_surfaces = [deg2rad(5);deg2rad(2);deg2rad(-13);(0.3)] + [0.1079;0;0;0.3182];; % given as u0 in problem


% Call the ODE solver for the aircraft equations of motion
[t, state_vectorT] = ode45(@(t, y) AircraftEOM(time, aircraft_state_array, aircraft_surfaces, wind_inertial, aircraft_parameters), time, Aircraft_State0);

% call PlotAircraftSim
fig = [1000,2000,3000,4000,5000,6000];

state_vectorT = state_vectorT';
PlotAircraftSim(t, state_vectorT, repmat(aircraft_surfaces, 1, numel(t)), fig, '-r');
...
[t,x] = ode45(@(t,x) AircraftEOMDoublet(time, Aircraft_State0, aircraft_surfaces, doublet_size,doublet_time, wind_inertial, aircraft_parameters), time, Aircraft_State0);

% Call PlotAircraftSim for the doublet response
fig = [301, 302, 303, 304, 305, 306];
x = x';
